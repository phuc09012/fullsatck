...
const reportView = ref('summary');
const accountView = ref('list');
...
const accountForm = reactive(createDefaultAccountForm());
const externalSearch = reactive({
  query: '',
  results: [],
  loading: false
});
const themeMode = ref(localStorage.getItem('library-theme') || 'dark');
const themeCordDragging = ref(false);
const themeCordOffset = ref(0);
const themeCordX = ref(0);
const themeCordReleaseTimer = ref(null);
const themeCordIgnoreClick = ref(false);
const themeCordDidDrag = ref(false);
const themeCordPath = computed(() => {
  const lift = Math.max(-1, Math.min(1, themeCordOffset.value / 48));
  const sway = Math.max(-1, Math.min(1, themeCordX.value / 26));
  const bend = sway * 26;
  const endX = 30 + bend;
  const endY = 212 + lift * 16;
  const slack = 12 - lift * 8;

  return `M 30 0
    C ${30 + bend * 0.08} ${18 + slack * 0.25}, ${30 + bend * 0.65} ${44 + slack * 0.55}, ${30 + bend * 0.78} ${82 + slack * 0.9}
    C ${30 + bend * 0.88} ${116 + slack * 1.15}, ${30 + bend * 0.96} ${150 + slack * 1.42}, ${endX} ${endY}`;
});
const themeCordKnobY = computed(() => 212 + Math.max(-1, Math.min(1, themeCordOffset.value / 48)) * 16);
const themeCordKnobX = computed(() => 30 + Math.max(-1, Math.min(1, themeCordX.value / 26)) * 26);
...
const visibleSections = computed(() => {
  if (isReader.value) {
    return [
      ...
      { key: 'borrows', label: 'Mượn trả sách' }
    ];
  }
  ...
  if (isAdmin.value) {
    sections.push({ key: 'accounts', label: 'Tài khoản' });
  }
...
function createDefaultAccountForm() {
  return {
    userId: '',
    fullName: '',
    email: '',
...
function resetAccountForm(user = null) {
...
function cloneBookForm(book = null) {
  ...
  return {
    ...
    maxBorrowingsPerReader: book.maxBorrowingsPerReader ?? 1,
...
function handleThemeToggleClick() {
  if (themeCordIgnoreClick.value) {
    themeCordIgnoreClick.value = false;
    return;
  }
  toggleTheme();
}

function setThemeCordOffset(offset, xOffset = 0) {
  themeCordOffset.value = Math.max(-40, Math.min(48, offset));
  themeCordX.value = Math.max(-26, Math.min(26, xOffset));
}

function startThemeCordDrag(event) {
  themeCordDragging.value = true;
  themeCordDidDrag.value = false;
  if (themeCordReleaseTimer.value) {
    clearTimeout(themeCordReleaseTimer.value);
    themeCordReleaseTimer.value = null;
  }
  event.currentTarget.setPointerCapture?.(event.pointerId);
  const startY = event.clientY;
  const startX = event.clientX;
  const startOffset = themeCordOffset.value;
  const startXOffset = themeCordX.value;

  const handleMove = (moveEvent) => {
    const deltaY = moveEvent.clientY - startY;
    const deltaX = moveEvent.clientX - startX;
    const offset = startOffset + deltaY * 0.9;
    const xOffset = startXOffset + deltaX * 0.35 + Math.sin((deltaY + deltaX) / 38) * 6;
    if (Math.abs(offset - startOffset) > 6 || Math.abs(xOffset - startXOffset) > 4) {
      themeCordDidDrag.value = true;
    }
    setThemeCordOffset(offset, xOffset);
  };
  ...
  const finish = () => {
    ...
    themeCordDragging.value = false;

    if (themeCordDidDrag.value && themeCordOffset.value < -8) {
      applyTheme('light');
    } else if (themeCordDidDrag.value && themeCordOffset.value > 8) {
      applyTheme('dark');
    } else {
      themeCordOffset.value = 0;
      themeCordX.value = 0;
    }

    themeCordIgnoreClick.value = themeCordDidDrag.value;

    themeCordReleaseTimer.value = window.setTimeout(() => {
      themeCordOffset.value = 0;
      themeCordX.value = 0;
      themeCordReleaseTimer.value = null;
      themeCordIgnoreClick.value = false;
...
function applyExternalBook(candidate) {
  bookForm.isbn = candidate.isbn?.trim() || `OL-${candidate.sourceId || crypto.randomUUID()}`;
...
const totalBookPages = computed(() => Math.max(1, Math.ceil(visibleBooks.value.length / booksPerPage)));
const pagedBooks = computed(() => {
  const start = (bookPage.value - 1) * booksPerPage;
  return visibleBooks.value.slice(start, start + booksPerPage);
});
...
async function loadStaffData() {
  ...
  try {
    ...
    if (accountView.value === 'editor' && selectedAccount.value) {
      const refreshed = users.value.find((user) => user.userId === selectedAccount.value.userId);
      if (refreshed) {
        resetAccountForm(refreshed);
      }
    }
  } catch (error) {
    setMessage('error', error.message);
...
async function submitBookForm() {
  ...
  try {
    ...
    const payload = {
      ...
      maxBorrowingsPerReader: Math.max(1, Number(bookForm.maxBorrowingsPerReader)),
...
async function deleteCategory(category) {
  if (auth.role !== 'Admin') {
    setMessage('error', 'Chỉ Admin mới được xóa thể loại sách.');
    return;
  }

  if (!category?.id) {
    setMessage('error', 'Không tìm thấy thể loại để xóa.');
    return;
  }

  const confirmed = window.confirm(
    `Xóa thể loại "${category.name}"? Chỉ xóa được khi không còn sách nào đang dùng thể loại này.`
  );
  if (!confirmed) {
    return;
  }

  busyAction.value = `category-delete:${category.id}`;
  try {
    await apiRequest(`/api/book-categories/${category.id}`, {
      method: 'DELETE',
      headers: { Authorization: `Bearer ${auth.token}` }
    });

    setMessage('success', `Đã xóa thể loại "${category.name}".`);
    await loadBookCategories();
    ...
    if (bookForm.category === category.name) {
      bookForm.category = '';
      ensureBookFormCategory();
    }
  } catch (error) {
...
async function submitAccountForm() {
  if (!isAdmin.value || !accountForm.userId) return;

  busyAction.value = 'account-form';
  try {
    const original = users.value.find((user) => user.userId === accountForm.userId);
    if (!original) {
      setMessage('error', 'Không tìm thấy tài khoản cần sửa.');
      return;
    }

    if (accountForm.role !== original.role) {
      await apiRequest(`/api/users/${accountForm.userId}/role`, {
        method: 'PUT',
        headers: { Authorization: `Bearer ${auth.token}` },
    ...
    if (accountForm.isActive !== original.isActive) {
      await apiRequest(`/api/users/${accountForm.userId}/status`, {
        method: 'PUT',
        headers: { Authorization: `Bearer ${auth.token}` },
        body: JSON.stringify({ isActive: accountForm.isActive })
      });
    }

    setMessage('success', 'Đã cập nhật tài khoản.');
    accountView.value = 'list';
    resetAccountForm();
    await loadStaffData();
  } catch (error) {
...
watch(
  ...
  () => {
    if (!auth.isAuthenticated) return;
        ...
        const allowed = visibleSections.value.map((section) => section.key);
        if (!allowed.includes(activeSection.value)) {
          activeSection.value = allowed[0] || 'books';
        }
  }
);
...
<template>
  <div class="dashboard-shell">
    <aside class="sidebar slide-in-left">
      ...
      <div class="sidebar-card" :class="{ 'sidebar-card--auth': !auth.isAuthenticated }">
    ...
    <div class="content">
      <header
        ...
        id="topbar"
        class="topbar fade-up"
        :class="{ 'topbar--solo': !isAdmin || !auth.isAuthenticated, 'topbar--public': !isAdmin || !auth.isAuthenticated }"
      >
        <div class="topbar-title">
          <p class="eyebrow">Dashboard</p>
          <h1>Quản lý thư viện theo thời gian thực</h1>
          <p class="topbar-copy" v-if="!isAdmin || !auth.isAuthenticated">
            Thư viện số giúp bạn tra cứu sách, theo dõi mượn trả và quản lý tài liệu nhanh gọn, thuận tiện ở mọi lúc mọi nơi.
          </p>
        </div>

        <div class="topbar-actions">
          <div class="topbar-action-row">
            <div class="theme-lamp">
              <div
                class="theme-toggle theme-toggle--hero"
                role="group"
                :class="{ dark: isDarkTheme, dragging: themeCordDragging }"
                :style="{ '--cord-offset': `${themeCordOffset}px`, '--cord-x': `${themeCordX}px` }"
              >
                <span
                  class="theme-bulb"
                  role="button"
                  ...
                  @keydown.enter.prevent="handleThemeToggleClick"
                  @keydown.space.prevent="handleThemeToggleClick"
                  :aria-pressed="isDarkTheme"
                  :aria-label="isDarkTheme ? 'Chuyển sang chế độ sáng' : 'Chuyển sang chế độ tối'"
                >
                  <span class="bulb-glow"></span>
                  <span class="bulb-base"></span>
                  <span class="bulb-ray bulb-ray-left"></span>
                  <span class="bulb-ray bulb-ray-right"></span>
                  <span class="bulb-ray bulb-ray-top"></span>
                  <span class="bulb-ray bulb-ray-bottom"></span>
                </span>
                <svg class="theme-wire" viewBox="0 0 60 220" preserveAspectRatio="none" @pointerdown.prevent.stop="startThemeCordDrag" @click.stop.prevent aria-hidden="true">
                  <path class="theme-wire-path" :d="themeCordPath" />
                  <circle class="theme-wire-knob" :cx="themeCordKnobX" :cy="themeCordKnobY" r="7" />
                </svg>
              </div>
              <p class="theme-caption">{{ isDarkTheme ? 'Kéo lên để bật sáng' : 'Kéo xuống để tắt' }}</p>
            </div>

            <button v-if="isStaff" class="notif-btn badge-pop" type="button" @click="jumpTo('transactions')">
              <span>Chuông</span>
              <strong>{{ staffNotifications }}</strong>
            </button>
          </div>
      ...
      <section v-if="activeSection === 'overview' && isAdmin" class="stats-grid fade-up" style="--delay: 90ms">
      ...
      <section v-if="activeSection === 'books'" id="books" class="section-block fade-up" style="--delay: 180ms">
        ...
        <div v-if="showBookForm && isStaff" class="book-form-panel scale-in">
          ...
          <div v-if="auth.role === 'Admin'" class="category-manager">
            ...
            <div class="category-chip-row">
              <div v-for="category in categoryOptions" :key="category.id" class="category-chip-item">
                <button
                  type="button"
                  class="category-chip"
          ...
          <form class="book-form" @submit.prevent="submitBookForm">
            <div class="form-grid">
              ...
              <label>
                <span>Ngưỡng mượn / độc giả</span>
                <input v-model="bookForm.maxBorrowingsPerReader" type="number" min="1" />
              </label>
              <label>
                <span>Ngưỡng tồn kho</span>
        ...
        <div v-else class="pagination-shell">
          <div class="pagination-row">
            <button class="ghost-button small" type="button" :disabled="bookPage <= 1" @click="bookPage -= 1">Trước</button>
            <span>Trang {{ bookPage }} / {{ totalBookPages }}</span>
            <button class="ghost-button small" type="button" :disabled="bookPage >= totalBookPages" @click="bookPage += 1">Sau</button>
          </div>
          ...
          >
            ...
            <div class="book-stock">
              ...
              <div class="stock-line">
                ...
                <span>Mượn / độc giả {{ book.maxBorrowingsPerReader }} · Tồn kho {{ book.minimumCopies }}</span>
              </div>
            </div>
      ...
      <section v-if="activeSection === 'transactions'" id="transactions" class="section-block fade-up" style="--delay: 270ms">
        <div class="section-head">
          <div>
      ...
      <section v-if="activeSection === 'reports'" id="reports" class="section-block fade-up" style="--delay: 360ms">
        <div class="section-head">
          <div>
        ...
        <div class="pill-row">
          <button class="filter-pill" :class="{ active: reportView === 'summary' }" type="button" @click="reportView = 'summary'">Tổng quan</button>
          <button class="filter-pill" :class="{ active: reportView === 'readers' }" type="button" @click="reportView = 'readers'">Độc giả</button>
          <button class="filter-pill" :class="{ active: reportView === 'overdue' }" type="button" @click="reportView = 'overdue'">Quá hạn</button>
        </div>

        <div v-if="reportView === 'summary' && isStaff && borrowingPolicy" class="info-card policy-card">
          <div class="panel-head">
            <div>
          ...
          <form class="policy-form" @submit.prevent="submitPolicyForm">
            ...
            <button v-if="isAdmin" class="primary-button" type="submit" :disabled="busyAction === 'policy-form'">
              {{ busyAction === 'policy-form' ? 'Đang lưu...' : 'Lưu quy tắc' }}
            </button>
          </form>
        </div>

        <div v-else-if="reportView === 'readers'" class="info-card">
          <h3>Độc giả gần đây</h3>
          <div class="info-list">
            <article v-for="reader in readers.slice(0, 6)" :key="reader.userId" class="info-row">
              <div>
        ...
        <div v-else-if="reportView === 'overdue'" class="info-card">
          <h3>Quá hạn cần xử lý</h3>
          <div class="info-list">
            <article v-for="row in recentBorrowingRows.filter(isOverdueBorrowing).slice(0, 6)" :key="row.id" class="info-row danger-row">
              <div>
                <strong>{{ row.bookTitle }}</strong>
      ...
      <section v-if="activeSection === 'borrows' && isReader" id="borrows" class="section-block fade-up" style="--delay: 360ms">
        <div class="section-head">
          <div>
            <p class="eyebrow">Mượn trả sách</p>
            <h2>Phiếu mượn của bạn</h2>
          </div>
          <span class="section-note">Tách riêng khỏi báo cáo</span>
        </div>
        ...
        <div class="info-card reader-borrow-card">
      ...
      <section v-if="activeSection === 'accounts' && isAdmin" id="accounts" class="section-block fade-up" style="--delay: 360ms">
        <div class="section-head">
          <div>
            <p class="eyebrow">Tài khoản</p>
            <h2>Quản lý tài khoản</h2>
          </div>
          <span class="section-note">{{ users.length }} tài khoản</span>
        </div>

        <div class="info-card">
          <div class="info-list">
            <article v-for="user in users" :key="user.userId" class="info-row">
              <div>
              ...
              <div class="reader-row-actions">
                <span class="row-badge" :class="user.isActive ? 'success' : 'danger'">{{ user.isActive ? 'Hoạt động' : 'Đã khóa' }}</span>
                <button class="ghost-button small" type="button" @click="openAccountEditor(user)">Sửa</button>
              </div>
            </article>
          </div>
        </div>

        <div v-if="accountView === 'editor'" class="info-card" style="margin-top: 18px;">
          <div class="panel-head">
            <div>
              <p class="eyebrow">Chỉnh sửa</p>
              <h3>{{ accountForm.fullName }}</h3>
            </div>
            <button class="ghost-button small" type="button" @click="cancelAccountEdit">Đóng</button>
          </div>
          <form class="book-form" @submit.prevent="submitAccountForm">
            <div class="form-grid">
              <label>
                <span>Họ và tên</span>
                <input v-model="accountForm.fullName" type="text" disabled />
              </label>
              <label>
                <span>Email</span>
              ...
              <label>
                ...
                <select v-model="accountForm.isActive">
            ...
            <div class="form-actions">
              <button class="primary-button" type="submit" :disabled="busyAction === 'account-form'">Lưu thay đổi</button>
            </div>
          </form>
        </div>
      </section>
