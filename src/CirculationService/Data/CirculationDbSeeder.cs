...
public static class CirculationDbSeeder
{
    public static async Task SeedAsync(CirculationDbContext context, CancellationToken cancellationToken = default)
    {
        ...
        await context.Database.ExecuteSqlRawAsync(
            ...
            IF OBJECT_ID('dbo.BorrowingPolicies', 'U') IS NULL
            BEGIN
        ...
        if (!await context.BorrowingPolicies.AnyAsync(x => x.Id == BorrowingPolicy.SingletonId, cancellationToken))
        {
            context.BorrowingPolicies.Add(new BorrowingPolicy
            {
                Id = BorrowingPolicy.SingletonId,
                MaxActiveBorrowingsPerReader = 5,
                DefaultBorrowDays = 14,
                MaxRenewalDays = 30,
                FinePerOverdueDay = 2000m,
